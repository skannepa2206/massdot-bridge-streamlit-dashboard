...
# ---- ROBUST MODEL LOADING WITH PROPER EMPTY FILE DETECTION ----
@st.cache_resource
def load_models_with_fallback():
    """Load models with dependency checking and proper empty file detection"""
        ...
        try:
            if os.path.exists(file_path):
                ...
                if file_size == 0:
                    ...
                    st.sidebar.error(f"❌ {file_path} is EMPTY - regenerate this file!")
                ...
                try:
                    ...
                    if loaded_model is None:
                        ...
                        st.sidebar.error(f"❌ {file_path} loaded as None")
                        continue
                    ...
                    st.sidebar.error(f"Load error for {display_name}: {str(load_error)}")
            ...
            st.sidebar.error(f"❌ Error with {display_name}: {str(e)}")
