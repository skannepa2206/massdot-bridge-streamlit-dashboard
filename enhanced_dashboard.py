...
# ---- ROBUST MODEL LOADING (NO FALLBACKS) ----
@st.cache_resource
def load_models_strict():
    """Load models with strict requirements - detailed debugging"""
    models = {'mlp': None, 'tabnet': None, 'scaler': None, 'features': None}
    loading_status = {'messages': [], 'success': 0, 'total': 4}
    
    # Primary model files only (no more backup files)
    required_files = [
    ...
    st.sidebar.write(f"🔍 **Debug - Files in models/:** {model_files}")
    
    # Load required models with detailed error reporting
        ...
        try:
            if os.path.exists(file_path):
                ...
                if model_type == "tabnet":
                    # Handle TabNet loading with specific error handling
                    try:
    ...
    if missing_models:
        st.error("🔴 **Critical Error: Missing Required Models**")
        st.write(f"**Missing:** {', '.join(missing_models)}")
        
        # Show expected vs found files
        st.write("**Expected Files vs Found:**")
        for file_path, model_type, display_name in required_files:
            exists = "✅" if os.path.exists(file_path) else "❌"
            st.write(f"- {exists} `{file_path}` ({display_name})")
        
        st.write("**All files in models/ directory:**")
        if os.path.exists("models"):
            for f in os.listdir("models"):
                st.write(f"- 📄 `models/{f}`")
        
        st.error("Please upload ALL required model files to proceed.")
        st.stop()
