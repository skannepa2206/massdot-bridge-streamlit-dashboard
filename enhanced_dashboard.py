...
# ---- ROBUST MODEL LOADING WITH DEPENDENCY HANDLING ----
@st.cache_resource
def load_models_with_fallback():
    """Load models with dependency checking and graceful fallback"""
    ...
    if missing_deps:
        ...
        # Show current requirements.txt if it exists
        if os.path.exists("requirements.txt"):
            with open("requirements.txt", "r") as f:
                current_reqs = f.read()
            st.write("**Current requirements.txt:**")
            st.code(current_reqs)
    ...
    st.sidebar.write(f"🔍 **Files in models/:** {model_files}")
    ...
    for file_path, model_type, display_name in required_files:
        try:
            if os.path.exists(file_path):
                ...
                st.sidebar.write(f"📁 {file_path}: {file_size:.1f}MB")
                
                # Check for empty files
                if file_size == 0:
                    loading_status['messages'].append(f"❌ {display_name}: File is empty (0MB)")
                    st.sidebar.error(f"❌ {file_path} is empty - needs to be regenerated")
                    continue
                ...
                if model_type == "scaler":
                    ...
                    try:
                        ...
                        loading_status['messages'].append(f"❌ {display_name}: sklearn error - {str(scaler_error)}")
                        st.sidebar.error(f"Scaler error: {str(scaler_error)}")
                    ...
                    try:
                        ...
                        loading_status['messages'].append(f"❌ {display_name}: sklearn error - {str(mlp_error)}")
                        st.sidebar.error(f"MLP error: {str(mlp_error)}")
                    ...
                    try:
                        ...
                        loading_status['messages'].append(f"✓ {display_name}: Loaded successfully")
                        loading_status['success'] += 1
                    except Exception as tabnet_error:
                        loading_status['messages'].append(f"❌ {display_name}: TabNet error - {str(tabnet_error)}")
                        st.sidebar.error(f"TabNet error: {str(tabnet_error)}")
                        
                else:
                    # Features and other files
                    models[model_type] = joblib.load(file_path)
                    loading_status['messages'].append(f"✓ {display_name}: Loaded successfully")
                    loading_status['success'] += 1
    ...
    if missing_models:
        st.error("🔴 **Critical Error: Model Loading Failed**")
        ...
        if empty_files:
            st.error(f"🔴 **Empty Files Detected**: {', '.join(empty_files)} are 0MB and need to be regenerated")
        
        # Check for sklearn errors
        sklearn_errors = [msg for msg in loading_status['messages'] if 'sklearn' in str(msg)]
        if sklearn_errors:
            st.error("🔴 **Missing scikit-learn**: Add 'scikit-learn' to requirements.txt")
        
        st.write(f"**Missing/Failed:** {', '.join(missing_models)}")
        ...
        2. **If files are 0MB, regenerate them:**
        ```python
        import joblib
        # Re-save your trained models
        joblib.dump(scaler, 'models/scaler.pkl')
        joblib.dump(features_list, 'models/features_used.pkl')
        ```
    ...
    st.sidebar.success(f"✅ All models loaded! ({loading_status['success']}/4)")
    ...
    for hour in hours:
        for minute in [0, 30]:
            ...
            try:
                ...
                # Get predictions from both models - handle array outputs safely
                mlp_pred = 0
                if mlp_model:
                    mlp_output = mlp_model.predict(scaled_features)
                    mlp_pred = float(mlp_output[0]) if hasattr(mlp_output, '__len__') else float(mlp_output)
                
                tabnet_pred = 0
                if tabnet_model:
                    try:
                        tabnet_output = tabnet_model.predict(scaled_features)
                        tabnet_pred = float(tabnet_output[0]) if hasattr(tabnet_output, '__len__') else float(tabnet_output)
                    except:
                        tabnet_pred = 0
            ...
            # Convert to scalars safely
            pred_hour = int(float(start_time) // 60) % 24
            pred_minute = int(float(start_time) % 60)
            duration = max(10, min(30, float(duration)))
...
if st.session_state.is_admin:
    ...
    with tab2:
        ...
        if is_real_data and len(hist_df) > 0:
            ...
            # Monthly lift trends
            st.markdown("**Monthly Bridge Lift Trends**")
            
            monthly_data = hist_df.groupby([hist_df['Start Time'].dt.year, hist_df['Start Time'].dt.month]).size().reset_index()
            monthly_data['date'] = pd.to_datetime(monthly_data[['Start Time', 'Start Time']].rename(columns={'Start Time': 'year', 'Start Time': 'month'}))
            monthly_data = monthly_data.rename(columns={0: 'lifts'})
